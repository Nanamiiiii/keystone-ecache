...
bool 
KeystoneDevice::copyCachedEnclave(struct runtime_params_t params, 
                                  unsigned char *sign,  unsigned char *pub_key, unsigned char *cert){
  struct keystone_ioctl_create_enclave encl;
  int i = 0;
  encl.eid           = eid;
  encl.params        = params;
  ...
  for(i = 0; i < ED_KEY_LENGTH/2; i++){
    encl.pub_key[i] = pub_key[i];
  }
  for(i = 0; i < CERT_SIZE/2; i++){
    encl.certificate[i] = cert[i];
  }
  
  if(ioctl(fd, KEYSTONE_IOC_COPY_CACHED_ENCLAVE, &encl)){//あればtrue
    return true;
  }
  return false;
}
...
Error
KeystoneDevice::cacheFinalize(
    uintptr_t runtimePhysAddr, uintptr_t eappPhysAddr, uintptr_t freePhysAddr,
    struct runtime_params_t params, unsigned char *sign, unsigned char *pub_key, 
    unsigned char *cert) {
  struct keystone_ioctl_create_enclave encl;
  int i = 0;
  encl.eid           = eid;
  ...
  if (ioctl(fd, KEYSTONE_IOC_FINALIZE_CACHE, &encl)) {
    perror("ioctl error");
    return Error::IoctlErrorFinalize;
  }
  cacheEid = eid;
...
Error
KeystoneDevice::resetcache(){
  if (ioctl(fd, KEYSTONE_IOC_DESTROY_ENCLAVE, 0)) {
    perror("ioctl error");
    return Error::IoctlErrorDestroy;
  }
