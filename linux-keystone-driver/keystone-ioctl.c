...
bool keystone_copy_cached_enclave(unsigned long arg){
  struct sbiret ret;
  ...
  //epmとutmの物理アドレスとサイズ、utmの仮想アドレスをsmに渡す
  /* SBI Call */
  ...
  for(i = 0; i < SIGN_SIZE; i++){
    create_args.sign[i] = enclp->sign[i];
  }
  for(i = 0; i < ED_KEY_LENGTH; i++){
    create_args.pub_key[i] = enclp->pub_key[i];
  }
  for(i = 0; i < CERT_SIZE; i++){
    create_args.certificate[i] = enclp->certificate[i];
  }

  ret = sbi_sm_copy_cached_enclave(&create_args);
...
int keystone_finalize_cache(unsigned long arg){
  struct sbiret ret;
  struct enclave *enclave;
  struct utm *utm;
  struct keystone_sbi_create_t create_args;
  ...
  ret = sbi_sm_create_cache(&create_args);
...
int keystone_reset_cache(void){
  struct sbiret ret;
  ret = sbi_sm_reset_cache();
  return 0;
}
...
long keystone_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
  ...
  switch (cmd) {
    case KEYSTONE_IOC_CREATE_ENCLAVE:
      ...
      break;
    case KEYSTONE_IOC_COPY_CACHED_ENCLAVE:
      ret = keystone_copy_cached_enclave((unsigned long) data);
      break;
    case KEYSTONE_IOC_FINALIZE_ENCLAVE:
      ret = keystone_finalize_enclave((unsigned long) data);
      break;
    case KEYSTONE_IOC_FINALIZE_CACHE:
      ret = keystone_finalize_cache((unsigned long)data);
      break;
    case KEYSTONE_IOC_DESTROY_ENCLAVE:
      ret = keystone_destroy_enclave(filep, (unsigned long) data);
      break;
    case KEYSTONE_IOC_RESET_CACHE:
      ret = keystone_reset_cache();
      break;
