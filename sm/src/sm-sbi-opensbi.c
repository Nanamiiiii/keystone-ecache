                     ...
                     struct sbi_trap_info *out_trap)
{
  ...
  switch (funcid) {
    case SBI_SM_CREATE_ENCLAVE:
      ...
      break;
    case SBI_SM_CREATE_CACHE:
      retval = sbi_sm_create_cache(out_val, regs->a0);
      break;
    case SBI_SM_COPY_CACHED_ENCLAVE:
      retval = sbi_sm_copy_cached_enclave(out_val, regs->a0);
      break;
    case SBI_SM_DESTROY_ENCLAVE:
      retval = sbi_sm_destroy_enclave(regs->a0);
      break;
    case SBI_SM_RESET_CACHE:
      retval = sbi_sm_reset_cache();
