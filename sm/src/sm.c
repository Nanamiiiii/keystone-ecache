...
int enclave_cache_init()
{
  int region = -1;
  int ret = pmp_region_init_atomic(ENCLAVE_CACHE_BASE, ENCLAVE_CACHE_SIZE, PMP_PRI_ANY, &region, 0);
  if(ret)
    return -1;
...
int sm_verify(void* certificate, void* message, size_t len, const unsigned char* public_key){
  return verify(certificate, message, len, public_key);
}
...
void sm_init(bool cold_boot)
{
  ...
  if (cold_boot) {
    ...
    enclave_cache_id = enclave_cache_init();
    if(enclave_cache_id < 0) {
      sbi_printf("[SM] intolerable error - failed to initialize enclave cache memory");
      sbi_hart_hang();
    }
  ...
  pmp_set_keystone(enclave_cache_id, PMP_NO_PERM);
