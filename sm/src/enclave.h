...
struct enclave_cache
{
  enclave_state state; // global state of the enclave cache
  ...
  //for cache
  byte signature[SIGN_SIZE];
  byte pub_key[ED_KEY_LENGTH];
  int rt_block;//RTの最初のブロックの位置
  int eapp_block;//eappの最初のブロックの位置
  int free_block;//freememの最初のブロックの位置
  int block_num;//使用しているブロック数
  short int first_block; //最初のブロックの位置
...
unsigned long create_cache(unsigned long *eid, struct keystone_sbi_create create_args);
unsigned long copy_cached_enclave(unsigned long *eid, struct keystone_sbi_create create_args_local);
int check_enclave_cache(byte *signature, byte *pub_key, int num);
unsigned long destroy_enclave(enclave_id eid);
unsigned long destroy_cache();
unsigned long reset_cache(void);
